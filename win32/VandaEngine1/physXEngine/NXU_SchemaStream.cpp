...
public:
  SchemaXML(const char *fname,void *mem,int len,SchemaStream *ss)
  {
    ...
    for (int i=0; i<SC_LAST; i++)
    {
	  ...
	  {
			//char temp[512];
			//sprintf(temp,"\nError	parsing file '%s' as XML", fname);
			//PrintInfo( temp, COLOR_RED );

			//char temp2[512];
			//sprintf(temp2,"\nXML	parse	error(%d)	on Row:	%d Column: %d",	mDocument->ErrorId(), mDocument->ErrorRow(), mDocument->ErrorCol());
			//PrintInfo( temp2, COLOR_RED );

			//char temp3[512];
			//sprintf(temp3,"\nXML	error	description: \"%s\"",	mDocument->ErrorDesc());
			//PrintInfo( temp3, COLOR_RED );
  ...
  ~SchemaXML(void)
  {
	...
	bool isClass(const char *str,SCHEMA_CLASS &t,SchemaClass *last,TiXmlNode *node)
	{
		...
		{
			//PrintInfo("\nThis old version of NxuStream is no longer supported.", COLOR_YELLOW);
		}
    ...
    if ( isBinary )
    {
      ...
      if ( mFph )
      {
        ...
        if ( strcmp(stream,"NXUSTREAM") != 0 )
        {
          ok = false;
		//  char temp[512];
        //  sprintf(temp, "\nFile %s is not a valid binary NxuStream file, missing header", fname );
		//  PrintInfo( temp);
        }
        ...
        if ( ok && uv != NXUSTREAM_VERSION )
        {
          ok = false;
		 // char temp[512];
         // sprintf(temp, "\nEven though the file %s was saved with the same SDK version, the version format has changed from %d to %d",fname,uv,NXUSTREAM_VERSION );
		 // PrintInfo( temp);
        }
      ...
      {
		//  char temp[512];
       //   sprintf(temp, "\nFailed to open binary file '%s' for read access.", fname );
		//  PrintInfo( temp);
      }
