...
CNovodex::CNovodex()
{
	m_hasExternalScene = CFalse; //imported scene from 3ds Max or Maya-No longer supported
	m_groundBox = NULL;
...
CBool CNovodex::initNx( CFloat XCharacterPos, CFloat YCharacterPos, CFloat ZCharacterPos, CFloat crtlRadius, CFloat crtlHeight, CFloat crtSlopeLimit, CFloat crtSkinWidth, CFloat crtStepOffset, CBool createScene )
{
	...
	if( createScene )
	{
		...
		if( !gPhysicsSDK )
		{
			MessageBox( NULL, _T("Couldn't initialize physX"), _T("Vanda Engine Error"), MB_OK | MB_ICONINFORMATION );
			return false;
		}
