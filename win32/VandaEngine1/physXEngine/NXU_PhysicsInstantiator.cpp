        ...
        stm->copyTo(desc,cc);
				...
				else
				{
					if ( callback )
						...
						//PrintInfo( "Failed to create CCD skeleton.", COLOR_YELLOW );
	...
	for	(NxU32 i = 0;	i	<	clothSize;	++i)
	{
        ...
        cmd->copyTo(desc,cc);
				...
				if ( ok )
				{
					...
					{
						//PrintInfo("\nFailed to cook cloth mesh", COLOR_YELLOW);
					}
					...
					else
					{
						...
						//PrintInfo("\nCloth Mesh failed to be created.", COLOR_YELLOW);
					}
				}
			}
	...
	for	(NxU32 i = 0;	i	<	softBodySize;	++i)
	{
        ...
        cmd->copyTo(desc,cc);
				...
				if ( ok )
				{
					...
					{
						//PrintInfo("\nFailed to cook softBody mesh", COLOR_YELLOW);
					}
					...
					else
					{
						...
						//PrintInfo("\nSoftBody Mesh failed to be created.", COLOR_YELLOW);
					}
				}
			}
        ...
        if ( tmd->mPmapData.size() )
        {
  		...
  		if (tm)
			...
			else
			{
				...
				//PrintInfo("\nFailed to create TriangleMesh", COLOR_YELLOW);
			}
		}
	}
			...
			if (hf ==	0)
			{
				...
				if ( ok )
				{
					...
					else
					{
						...
						//PrintInfo("\nFailed to create HeightField", COLOR_YELLOW);
					}
				}
			}
			hfd->mInstance = hf;
...
//*** Instance materials
	...
	for	(NxU32 i = 0;	i	<	count; ++i)
	{
		...
		if (callback)
			ok = callback->NXU_preNotifyMaterial(desc, mat->mUserProperties, mat->mMaterialIndex);
    ...
    if ( actor->mHasBody )
    {
      ...
      if ( shapeDesc )
      {
          ...
          slist.push_back(shapeDesc);
				...
				{
					//char temp[512];
					//sprintf(temp, "\nFailed to construct valid shape descriptor type(%s) for Actor (%s)", EnumToString( shape->mType) , actor->mId );
			...
			{
					//char temp[512];
					//sprintf(temp, "\nUnable to construct valid shape descriptor type(%s) for Actor (%s)", EnumToString( shape->mType) , actor->mId );
					//PrintInfo( temp, COLOR_YELLOW );
			}
			...
			if (ok)
			{
              ...
              sd->mInstance = shapes[i];
				...
				else
				{
					...
					//char temp[512];
					//sprintf(temp, "\nFailed to create actor '%s'", desc.name );
					//PrintInfo( temp, COLOR_YELLOW );
				}
			}
		}
...
//******************************************* do contraints
		...
		if ( jdesc )
		{
  		...
  		if (ok)
  		{
  			...
  			else
  			{
					...
					//char temp[512];
					//sprintf(temp, "\nFailed to create joint '%s'", v->name );
					//PrintInfo( temp, COLOR_YELLOW );
  			}
  		}
		}
    ...
    if ( ok )
    {
      ...
      else
      {
		...
		//char temp[512];
		//sprintf(temp,"Failed to create SpringAndDamper Effector '%s'\r\n", getStr(d->mId) );
...
// force field linear kernels
#if NX_SDK_VERSION_NUMBER >= 280
  {
	...
	for	(NxU32 i = 0; i<count; i++)
	{
		...
		{
			//char temp[512];
			//sprintf(temp,"\nFailed to create ForceField LinearKernel '%s'", getStr(d->mId) );
			//PrintInfo( temp, COLOR_RED );
		}
	}
  }
...
// force field groups
#if NX_SDK_VERSION_NUMBER >= 280
  {
	...
	for	(NxU32 i = 0; i<count; i++)
	{
		...
		{
			//char temp[512];
			//sprintf(temp,"\nFailed to create ForceField shape group '%s'", getStr(d->mId) );
			//PrintInfo( temp, COLOR_RED );
		}
	}
  }
	...
	for	(NxU32 i = 0;	i	<	count; ++i)
	{
		...
		if ( ok )
		{
			...
			else
			{
				...
				//char temp[512];
				//sprintf(temp,"\nFailed to create ForceField Effector '%s'", getStr(d->mId)  );
				//PrintInfo( temp, COLOR_RED );
			}
              ...
              //bool shapeOk = true;
                ...
                if ( a )
                {
  				...
  				else
  				{
  					...
  					//PrintInfo("\nFailed to create cloth", COLOR_YELLOW);
  				}
  			}
  		}
		}
              ...
              // bool shapeOk = true;
                ...
                if ( a )
                {
  				...
  				else
  				{
  					...
  					//PrintInfo("\nFailed to create soft body", COLOR_YELLOW);
  				}
  			}
  		}
		}
	...
	for	(NxU32 i = 0;	i	<	count; ++i)
	{
		...
		if (ok)
		{
			...
			else
			{
				...
				//PrintInfo("Failed to create fluid.", COLOR_YELLOW);
			}
		}
	}
#endif
			...
			else
			{
				...
				if (ok)
				{
					...
					else
					{
						...
						//PrintInfo("\nFailed to create NxCompartment", COLOR_YELLOW);
					}
				}
			}
		}
																							...
																							const NxMat34	*defaultSceneOffset)
{
    ...
    cc.initScene();
		...
		if (p)
		{
      	...
      	{
			//char temp[512];
			//sprintf(temp, "\nScene Descriptor had an invalid maxTimestep of %f; changing it to 0.02", sdesc.maxTimestep );
			//PrintInfo( temp, COLOR_YELLOW );
      	...
      	{
			//char temp[512];
			//sprintf(temp, "\nScene Descriptor had an invalid maxIter value of %d; changing it to the default of 8", sdesc.maxIter );
			//PrintInfo( temp, COLOR_YELLOW );
      		sdesc.maxIter = 8;
      	}
				...
				if ( !isValid )
				{
					//PrintInfo("\nSupplied scene descriptor is invalid.  Switching to the default settings.", COLOR_YELLOW);
					::NxSceneDesc def;
					sdesc = def;
				}
			...
			else
			{
				if ( defaultScene == 0 )
				{
					...
					//char temp[512];
					//sprintf( temp, "\nFailed to create scene '%s'", p->mId );
					//PrintInfo( temp, COLOR_YELLOW );
				}
			}
		}
  ...
  if ( callback )
	...
	if ( ok )
	{
		...
		else
		{
  		...
  		//PrintInfo("\nFailed to create convex mesh", COLOR_YELLOW);
  	}
  }
