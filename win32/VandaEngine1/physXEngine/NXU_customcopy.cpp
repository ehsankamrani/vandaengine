  	...
  	for (unsigned int i=0; i<mCurrentCollection->mConvexMeshes.size(); i++)
  	{
  		...
  		if ( strcmp(sd->mId,name) == 0 )
  		{
  			ret = (NxConvexMesh *) sd->mInstance;
				...
				{
					//char temp[512];
					//sprintf("\nConvexMesh %s was not successfully created.", name );
	...
	for (int j = 0; j < nbShapes; j++)
	{
		...
		if ( shapeDesc )
		{
			...
			else
			{
				delete shapeDesc;
				//char temp[512];
				//sprintf("\nFailed to construct valid force field shape descriptor type(%s) for Field (%s)", EnumToString( fsd->mType) , forceFieldID );
				//PrintInfo( temp, COLOR_YELLOW );
		...
		else
		{
			delete shapeDesc;
			//char temp[512];
			//sprintf("\nUnable to construct valid shape descriptor type(%s) for Actor (%s)", EnumToString( fsd->mType) ,  forceFieldID );
			//PrintInfo( temp, COLOR_YELLOW );
		}
	}
