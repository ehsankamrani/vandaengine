	...
	else
	{
		...
		if( checkState == BST_CHECKED )
		{
			if( g_shadowProperties.m_shadowType == eSHADOW_SINGLE_HL && g_shadowProperties.m_enable && g_render.UsingShadowShader())
				MessageBox( "Note: Fog does not work because you have enabled dynamic shadow debug mode. To solve this problem, please go to 'Modify -> Shadow' and select another shadow type in the dialog box that appears.", "Vanda Engine 1 Warning", MB_OK | MB_ICONINFORMATION );
