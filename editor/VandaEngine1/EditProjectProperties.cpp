...
void CEditProjectProperties::OnOK()
{
	...
	else
	{
		...
		if( m_edit )
		{
			...
			CChar temp[256];
			sprintf(temp, "%s%s%s%s%s", "Vanda Engine 1.9.1 (", newProjectName, " - ", m_currentVSceneNameWithoutDot, ")");
		...
		CChar temp[256];
		sprintf(temp, "%s%s%s%s%s", "Vanda Engine 1.9.1 (", newProject->m_name, " - ", m_currentVSceneNameWithoutDot, ")");
		ex_pVandaEngine1Dlg->SetWindowTextA(temp);
