...
CVoid CNode::RenderModelsControlledByPhysX(CBool sceneManager, CNode* sceneRoot )
{
	for(CUInt i=0; i< m_instanceGeometries.size(); i++)
	{
		...
		{
			if (g_editorMode == eMODE_PREFAB || (g_editorMode == eMODE_VSCENE && g_currentInstancePrefab->GetRenderForQuery()) || (g_editorMode == eMODE_VSCENE && g_currentInstancePrefab->GetRenderForWaterQuery()) || (g_editorMode == eMODE_VSCENE && g_renderShadow))
			{
