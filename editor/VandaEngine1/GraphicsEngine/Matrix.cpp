...
CVoid CMatrix4x4Translate(CMatrix LMatrix, CVec4f translate)
{
	...
	// concatenate to previously passed in matrix 
...
CVoid CMatrix4x4Decomepose( const CMatrix srcMatrix, CVec3f translation, CMatrix rotationMat, CMatrix skewMat )
{
	CMatrix copyMat;
	CMatrixCopy( srcMatrix, copyMat ); 

	CMatrixLoadIdentity( rotationMat );
	CMatrixLoadIdentity( skewMat );
	translation.x = copyMat[M30]; 
	translation.y = copyMat[M31]; 
	translation.z = copyMat[M32];

	copyMat[M30] = copyMat[M31] = copyMat[M32] = 0.0f;
	copyMat[M03] = copyMat[M13] = copyMat[M23] = 0.0f;
	copyMat[M33] = 1.0f;
	...
	CFloat norm;
    int count = 0;
	CMatrixCopy( copyMat, rotationMat  ); 
	do{
		CMatrix nextRotation;
		CMatrix currInvTranspose;
		CMatrix tempTranspose;
		CMatrixLoadIdentity( currInvTranspose );
		CMatrixLoadIdentity( nextRotation );
		CMatrixLoadIdentity( tempTranspose );

		CMatrixTranspose( rotationMat, tempTranspose );
		CMatrix3x4Invert( tempTranspose, currInvTranspose );
		// Go through every component in the matrices and find the next matrix
        for (int i = 0; i < 16; i++)
		{
			nextRotation[i] = 0.5 * (rotationMat[i]+currInvTranspose[i]);
		}
		norm = 0.0f;

		float n = fabs(rotationMat[00] - nextRotation[00]) + fabs(rotationMat[01] - nextRotation[01]) + fabs(rotationMat[02] - nextRotation[02]);
        norm = std::max<CFloat>(norm, n);
		n = fabs(rotationMat[10] - nextRotation[10]) + fabs(rotationMat[11] - nextRotation[11]) + fabs(rotationMat[12] - nextRotation[12]);
        norm = std::max<CFloat>(norm, n);
		n = fabs(rotationMat[20] - nextRotation[20]) + fabs(rotationMat[21] - nextRotation[21]) + fabs(rotationMat[22] - nextRotation[22]);
        norm = std::max<CFloat>(norm, n);
		CMatrixCopy(nextRotation, rotationMat);
		count++;
	}while( count < 100 && norm >0.001f );
	// The scale is simply the removal of the rotation from the non-translated matrix
	CMatrix inverseRotationMat;
	CMatrixLoadIdentity( inverseRotationMat );
	CMatrix3x4Invert( rotationMat, inverseRotationMat );
	CMatrix4x4Mult( inverseRotationMat, copyMat );
	CMatrixCopy( copyMat, skewMat );
	CChar temp[1024];
	sprintf( temp, "\n%f %f %f %f, %f %f %f %f, %f %f %f %f, %f %f %f %f", rotationMat[0], rotationMat[1], rotationMat[2], rotationMat[3], rotationMat[4], rotationMat[5], rotationMat[6], rotationMat[7], rotationMat[8], rotationMat[9], rotationMat[10],rotationMat[11], rotationMat[12],  rotationMat[13], rotationMat[14],rotationMat[15]);  
	PrintInfo( temp );

	CChar temp2[1024];
	sprintf( temp2, "\n%f %f %f %f, %f %f %f %f, %f %f %f %f, %f %f %f %f", skewMat[0], skewMat[1], skewMat[2], skewMat[3], skewMat[4], skewMat[5], skewMat[6], skewMat[7], skewMat[8], skewMat[9], skewMat[10],skewMat[11], skewMat[12],  skewMat[13], skewMat[14],skewMat[15]);  
	PrintInfo( temp2 );
