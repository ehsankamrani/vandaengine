...
CVoid CController::FindJointNodes( CNode * sceneRoot ) 
{
	...
	for ( CInt i = 0; i < m_jointCount; i++)
	{
		...
		if ( m_joints[i].m_jointNode )
		{
			...
			sprintf( temp, "\nSkin Controller (%s) joint binding made to node (%s) ", m_name, m_joints[i].m_name );
		...
		{
			sprintf( temp, "\nError! Failed to make joint binding for Controller %s for Joint %s", m_name, m_joints[i].m_name );
