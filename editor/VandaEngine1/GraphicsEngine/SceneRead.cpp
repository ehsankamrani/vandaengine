...
CImage *CScene::ReadImage( domImageRef lib )
{
	...
	if ( imageElement )
	{
		...
		if( g_useOriginalPathOfDAETextures && image ) //save functions. We update the new textures while importing the scenes
			newImage = image; //update the previous image
		else
			newImage = CNew( CImage );
		//CAssert( "No memory\n", newImage != NULL );
		...
		else if( ! (g_useOriginalPathOfDAETextures && image) ) //save functions. We update the new textures while importing the scenes
		{
			CDelete( newImage ); 
			return NULL; 
		}
...
CEffect *CScene::ReadEffect( domEffectRef lib )
{
	...
	if (effect)	// effect found
		return effect;
	PrintInfo( "\nAdding new Effect: '" );  
	PrintInfo( lib->getId(), COLOR_RED_GREEN );
	PrintInfo( "'");
		...
		for ( CUInt p = 0; p < numProfiles;  p++ )
		{
			...
			if ( Cmp("profile_COMMON", typeName ) )
			{
				...
				if( g_importColladaImages )
				{
					...
					if (image)
					{
						PrintInfo( "\nAttaching image '" );PrintInfo( image->GetFileName(), COLOR_RED_GREEN );
				...
				PrintInfo( "\nEffect added successfully", COLOR_WHITE );
				return newEffect; 	
			}
			else
			{
				...
				sprintf( tempChar, "\nCouldn't add the effect : %s . It's not supported", typeName );
				PrintInfo( tempChar );
			}
		}
	}
...
CMaterial *CScene::ReadMaterial( domMaterialRef lib )
{
	...
	if ( materialElement ) 
	{
		...
		if (effect) 
		{
			...
			PrintInfo( "\nAttaching effect '" );PrintInfo( effect->GetName(), COLOR_RED_GREEN );
			PrintInfo( "' to Material ' " );PrintInfo( newMat->GetName(),COLOR_RED_GREEN ); PrintInfo(" '"); 
			m_materials.push_back(newMat);
			PrintInfo( "\nMaterial added successfully", COLOR_WHITE );
			return newMat; 	
		}
		else
		{
			sprintf( tempChar, "\nError! No effect ' %s ' for old-style matrial ' %s '", element->getID(), lib->getId() ); 
...
CVoid CScene::ReadAnimationClip( domAnimationClipRef animationClip )
{
	...
	if ( animationClipElement ) 
	{
		...
		if( animationClipElement->getId() ) PrintInfo( animationClipElement->getId(), COLOR_RED_GREEN );  
		else PrintInfo( "" );
		PrintInfo( " '" );
		...
		for ( CUInt i = 0; i < animationClipElement->getInstance_animation_array().getCount(); i++)
		{
			...
			if( animation )
			{
				PrintInfo( "\nAttaching animation ' ", COLOR_WHITE );PrintInfo( animation->GetName(), COLOR_RED_GREEN );
				PrintInfo( " ' to the clip ' ", COLOR_WHITE  );PrintInfo(  animationClipElement->getId(), COLOR_RED_GREEN );
...
CAnimation *CScene::ReadAnimation( domAnimationRef animation, CBool loadTempAnimations )
{
	...
	if ( animation )
	{
		...
		if( animation->getId() ) PrintInfo( animation->getId(), COLOR_RED_GREEN );  
		else PrintInfo( "" );
		PrintInfo( " '" );
...
CBool CScene::ReadScene( domVisual_sceneRef scene, CUpAxis up )
{
	...
	//}
	PrintInfo( "\nReading external Scene '" );
	PrintInfo( scene->getName(), COLOR_RED_GREEN );
	PrintInfo("'..."); 	
...
CNode * CScene::ReadNode( domNodeRef node, CNode * parentNode )
{
	...
	PrintInfo( "\nReading Scene Node ' ");
	PrintInfo( node->getId(), COLOR_RED_GREEN );
	PrintInfo( " ' " );
	...
	for (CUInt i = 0; i < node->getInstance_geometry_array().getCount(); i++)
	{
		...
		PrintInfo( "\nGeometry added successfully", COLOR_WHITE );
	}
	...
	for (CUInt i = 0; i < node->getNode_array().getCount(); i++)
	{
		...
		PrintInfo( "\n   # Childern of node: ' "  );
		PrintInfo( tempNode->GetId(), COLOR_RED_GREEN );
		PrintInfo( " ' : " );
...
CInstanceMaterial *CScene::ReadInstanceMaterial( domInstance_materialRef lib)
{
	...
	//{   // can't resolve fx materials, try common materials
		...
		if (element)
		{
			...
			{
				CChar temp[ MAX_NAME_SIZE ];
				sprintf( "\nError! CScene::ReadInstanceMaterial : Couldn't find instance Materials for binding: %s",newInstanceMaterial->m_target.c_str() );
...
CGeometry *CScene::ReadGeometry( domGeometryRef lib , CController * controller)
{
	...
	if (!g_shareGeometriesBetweenScenes) //prefab mode
	{
		if (!controller)
			geometry = GetGeometry(lib->getID(), lib->getDocumentURI()->getURI());
	}
	else
	{
		if (!controller)
			geometry = GetGeometryFromScenes(lib->getID(), g_currentPrefabAndSceneName);
	}
	if (geometry)	// geometry found
		return geometry;
	...
	if(assetElement)
	{
		sprintf( tempChar, "\nWarning! CScene::ReadGeometry: Item %s has an <asset> which engine doesn't supports it", lib->getId() );
		PrintInfo( tempChar, COLOR_RED );
		numWarnings += 1;
	...
	{
		sprintf( tempChar, "\nWarning! CScene::ReadGeometry: Item %s has an <convex_mesh> which engine doesn't support supports it", lib->getId() );
		PrintInfo( tempChar, COLOR_RED );
		numWarnings += 1;
	...
	{
		//sprintf( tempChar, "\nWarning! CScene::ReadGeometry: Item %s has an <extra> which engine doesn't supports it", lib->getId() );
		//PrintInfo( tempChar, COLOR_YELLOW );
		//numWarnings += 1;
	}
	else if (splineElement)
	{
		sprintf( tempChar, "\nWarning! CScene::ReadGeometry: Item %s has an <spline> which engine doesn't supports it", lib->getId() );
		PrintInfo( tempChar, COLOR_YELLOW );
		numWarnings += 1;
	}
	...
	if( mesh )
	{
		...
		if (!g_shareGeometriesBetweenScenes) //prefab mode
		{
			newGeo->SetDocURI(lib->getDocumentURI()->getURI());
		}
		else
		{
			newGeo->SetDocURI(g_currentPrefabAndSceneName);
		}
		...
		for (CUInt i=0; i< numTriangleGroups; i++)
		{
			...
			newGeo->m_groups[i]->m_nameIndex = g_nameIndex++;
		}
...
public:
	COffsets(domInputLocalOffset_Array & inputs)
	{
		...
		for ( int i = 0; i < MAX_TEX_COORDS; i++ )
		{
			...
			texture_offset[i] = -1;
			texture_stride[i] = 2; //By default I assume that we have 2 texture coordinates: S and T.
		}
...
CInstanceLight *CScene::ReadInstanceLight( domInstance_lightRef lib)
{
	...
	{
		CChar tempChar[ MAX_NAME_SIZE ];
		sprintf(tempChar,"\nError! Can't find the light ' %s '", urltype.getURI() );
		PrintInfo( tempChar, COLOR_RED );
		numErrors += 1;
		return NULL;
	}
...
CInstanceCamera *CScene::ReadInstanceCamera( domInstance_cameraRef lib)
{
	xsAnyURI & urltype  = lib->getUrl();
	domElement * element = (domElement* ) urltype.getElement();
	if (element==NULL) // this instance camera is not found skip to the next one
	{
		CChar tempChar[ MAX_NAME_SIZE ];
		sprintf(tempChar,"\nCan't find the camera ' %s '", urltype.getURI() );
		PrintInfo( tempChar );
		return NULL;
	}
...
CCamera *CScene::ReadCamera( domCameraRef lib )
{
	...
	{	
		...
		if (!Cmp(g_currentInstancePrefabName, "\n"))
		{
		...
		{
			PrintInfo( "\nReading an orthographic camera" );
...
{
	if (lib->getId()==NULL) return NULL;
	PrintInfo( "\nAdding new controller ' " );PrintInfo( lib->getId(), COLOR_RED_GREEN );PrintInfo( " '" );
	...
	{
		PrintInfo( "\nError! CScene::ReadController: Critical document problem, Couldn't find the compatible array type for the joints.", COLOR_RED );
		numErrors += 1;
...
CVoid CScene::CalculateTangentArray(CGeometry* geometry)
{
	if( geometry->m_tangents || !geometry->m_texCoords[0])
		return;
	geometry->m_bNormals = CNewData( CVec3f, geometry->m_vertexcount );
	geometry->m_tangents = CNewData( CVec3f, geometry->m_vertexcount );

	CVec3f* tangentTemp = CNewData( CVec3f, geometry->m_vertexcount );
