...
CVoid COpenGLUtility::InitGLEW()
{
	GLenum err = glewInit();
	if( err != GLEW_OK )
		PrintInfo( _T("\nCouldn't initialize GLEW. Disabling extensions..."), COLOR_RED );
	else
		PrintInfo( _T("\nGLEW initialized successfully"), COLOR_WHITE );
}
...
CVoid COpenGLUtility::GetGLInfo()
{
	...
	PrintInfo( "\nDetecting OpenGL information...", COLOR_RED_GREEN );
	PrintInfo( "\nOpenGL Vendor: " + m_strVendor, COLOR_RED_GREEN );
	PrintInfo( "\nOpenGL Renderer: " + m_strRenderer, COLOR_RED_GREEN);
	PrintInfo( "\nOpenGL Version: " + m_strVersion, COLOR_RED_GREEN );
	PrintInfo( "\nOpenGL Shading Language Version: " + m_strShadingVersion, COLOR_RED_GREEN );
}
...
GLvoid COpenGLUtility::DrawSphere( GLdouble radius, GLint slices, GLint stacks, CFloat* pos, CFloat* diffuseColor )
{
	glUseProgram(0);
...
GLvoid COpenGLUtility::DrawSolidSphere(GLdouble radius, GLint slices, GLint stacks, CFloat* pos, CFloat* diffuseColor)
{
	glUseProgram(0);
...
GLvoid COpenGLUtility::DrawCone( GLdouble base, GLint height, GLint slices, GLint stacks, CFloat* diffuseColor )
{
	glUseProgram(0);
...
GLvoid COpenGLUtility::DrawCube( GLdouble size, CFloat* pos )
{
	glUseProgram(0);
