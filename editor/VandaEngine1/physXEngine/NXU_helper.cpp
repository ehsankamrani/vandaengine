...
#include <PhysX/NxVersionNumber.h>
#include <PhysX/NxPhysics.h>
#include <PhysX/NxPMap.h>
#include <PhysX/NxSceneStats.h>
...
// save	the	contents of	this collection	in the COLLADA format.
{
	...
	{
		char temp[512];
		sprintf(temp, "\nFailed to	open file	'%s' for write access.COLLADA	file not written.",	fname);
...
NxuPhysicsCollection *loadCollection(const char	*fname,	NXU_FileType type,void *mem,int len)
{
	...
	if ( fph )
	{
		...
		else
		{
			...
			if ( ss.isValid() )
			{
				...
				{
					char temp[512];
					sprintf(temp, "\nFailed to load collection '%s'", fname);
					PrintInfo( temp, COLOR_RED );
	...
	{
		char temp[512];
		sprintf(temp, "\nFailed to open physics data file '%s' for read access", fname);
		PrintInfo( temp, COLOR_RED );
	}
