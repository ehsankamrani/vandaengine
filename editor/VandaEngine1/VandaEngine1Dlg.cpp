...
ON_BN_CLICKED(IDC_BTN_GUI_IMAGE, &CVandaEngine1Dlg::OnBnClickedBtnGuiImages)
...
ON_BN_CLICKED(IDC_BTN_VSCENE_SCRIPT, &CVandaEngine1Dlg::OnBnClickedBtnVSceneScript)
...
BOOL CVandaEngine1Dlg::OnInitDialog()
{
	...
	SetWindowText(_T("Vanda Engine 1.8.5"));
	...
	for( CUInt i = 0; i < g_projects.size(); i++ )
	{
		if( g_projects[i]->m_isActive )
		{
			...
			CChar temp[256];
			sprintf(temp, "%s%s%s%s%s", "Vanda Engine 1.8.5 (", g_projects[i]->m_name, " - ", m_currentVSceneNameWithoutDot, ")");
	...
	if (g_multipleView->GetInitError() || !g_render.m_shaderAvailable)
	{
		...
		PrintInfo( "\nVersion 1.8.5 initialized successfully" );
	//CAboutDlg dlgAbout;
	//dlgAbout.DoModal();
	ReleaseCapture();
...
BOOL CVandaEngine1Dlg::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	else if (wParam == ID_MODE_PREFAB || wParam == ID_MODE_PREFAB2)
	{
		if (g_editorMode != eMODE_PREFAB)
		{
			...
			CChar temp[256];
			sprintf(temp, "%s", "Vanda Engine 1.8.5 : Prefab Mode (Untitled)");
			ex_pVandaEngine1Dlg->SetWindowTextA(temp);
	...
	else if (wParam == ID_MODE_GUI || wParam == ID_MODE_GUI2)
	{
		if (g_editorMode != ID_MODE_GUI)
		{
			...
			CChar temp[256];
			sprintf(temp, "%s", "Vanda Engine 1.8.5 : GUI Mode (Untitled)");
			ex_pVandaEngine1Dlg->SetWindowTextA(temp);
...
CBool CVandaEngine1Dlg::OnMenuClickedNew( CBool askQuestion )
{
	...
	if (g_editorMode == eMODE_VSCENE)
	{
		for (CUInt i = 0; i < g_projects.size(); i++)
		{
			...
			{
				CChar temp[256];
				sprintf(temp, "%s%s%s%s%s", "Vanda Engine 1.8.5 (", g_projects[i]->m_name, " - ", "Untitled", ")");
				ex_pVandaEngine1Dlg->SetWindowTextA(temp);
				break;
			}
		}
...
CVoid CVandaEngine1Dlg::OnMenuClickedSaveGUIAs(CBool askQuestion)
{
	...
	if (result == IDOK)
	{
		...
		CChar temp[256];
		sprintf(temp, "%s%s%s", "Vanda Engine 1.8.5 : GUI Mode (", g_currentPackageAndGUIName, ")");
		ex_pVandaEngine1Dlg->SetWindowTextA(temp);
...
//		//save functions////////////////////////////////////////
		...
		CChar temp[256];
		sprintf(temp, "%s%s%s", "Vanda Engine 1.8.5 : Prefab Mode (", g_currentPackageAndPrefabName, ")");
		ex_pVandaEngine1Dlg->SetWindowTextA(temp);
...
CBool CVandaEngine1Dlg::OnMenuClickedOpenGUI()
{
	...
	if (filePtr)
	{
		...
		for (CUInt i = 0; i < numberOfGUIImages; i++)
		{
			...
			ex_pVandaEngine1Dlg->InsertItemToGUIList(name, eGUILIST_IMAGE);
		...
		CChar temp[256];
		sprintf(temp, "%s%s%s", "Vanda Engine 1.8.5 : GUI Mode (", guiAndPackageName, ")");
		ex_pVandaEngine1Dlg->SetWindowTextA(temp);
...
CBool CVandaEngine1Dlg::OnMenuClickedOpenPrefab()
{
	...
	if (filePtr)
	{
		...
		g_updateOctree = CTrue;
		CChar temp[256];
		sprintf(temp, "%s%s%s", "Vanda Engine 1.8.5 : Prefab Mode (", prefabAndPackageName, ")");
		ex_pVandaEngine1Dlg->SetWindowTextA(temp);
...
CVoid CVandaEngine1Dlg::ChangeVSceneScriptProperties()
{
	...
	if (m_dlgAddVSceneScript->DoModal() == IDOK)
	{
		...
		ex_pVandaEngine1Dlg->InsertItemToEngineObjectList(g_VSceneScript->GetName(), eENGINEOBJECTLIST_VSCENESCRIPT);
		ex_pVandaEngine1Dlg->m_mainBtnVSceneScript.EnableWindow(FALSE);
		ex_pVandaEngine1Dlg->GetMenu()->EnableMenuItem(ID_INSERT_VSCENE_SCRIPT, MF_DISABLED);
		g_menu.m_insertVSceneScript = CTrue;
		g_selectedName = g_multipleView->m_lastSelectedName = g_tempLastEngineObjectSelectedName = g_lastEngineObjectSelectedName = g_multipleView->m_tempSelectedName = -1; 		//no object has been selected
...
void CVandaEngine1Dlg::OnBnClickedBtnGuiImages()
{
	if (g_multipleView->IsPlayGameMode())
	{
		if (MessageBox("You can not insert GUIs in Play Mode. Exit from play mode?", "Vanda Engine Error", MB_YESNO | MB_ICONINFORMATION) == IDYES)
...
CVoid CVandaEngine1Dlg::ChangeGUIImageProperties(CGUIImage* image)
{
	m_dlgAddGUIImage = CNew(CAddGUIImage);
	...
	if (result == IDOK)
	{
		...
		ex_pVandaEngine1Dlg->InsertItemToGUIList(m_dlgAddGUIImage->GetName(), eGUILIST_IMAGE);
	...
	if (nSelected >= 0)
	{
		...
		for (CUInt k = 0; k < g_guiImages.size(); k++)
		{
			if (Cmp(szBuffer, g_guiImages[k]->GetName()))
			{
				...
				ex_pVandaEngine1Dlg->m_staticSelectedObject.ReplaceSel(g_guiImages[k]->GetName());
...
void CVandaEngine1Dlg::OnBnClickedBtnVSceneScript()
{
	if (g_multipleView->IsPlayGameMode())
	{
		if (MessageBox("Exit from play mode?", "Vanda Engine Error", MB_YESNO | MB_ICONINFORMATION) == IDYES)
...
CVoid CVandaEngine1Dlg::OnMenuClickedInsertVSceneScript()
{
