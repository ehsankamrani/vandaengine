...
void CSelectCamera::OnBnClickedOk()
{
	...
	if( nSelected >= 0 )
	{
		...
		//	ex_pVandaEngineDlg->OnBnClickedBtnActivatePlayMode();
		//}
		...
		//{
			...
			for( CUInt i = 0; i < g_importedCameraInstances.size(); i++ )
			{
				if( Cmp( g_importedCameraInstances[i]->m_abstractCamera->GetName(), szBuffer ) )
				{
					...
					ex_pVandaEngineDlg->m_mainBtnFreeCamera.EnableWindow(TRUE);
			...
			//ex_pBtnDeactivatePlayMode->ShowWindow(SW_HIDE);
			//ex_pBtnDeactivatePlayMode->EnableWindow(FALSE);
			//ex_pBtnDeactivatePlayMode->UpdateWindow();
			//ex_pBtnActivatePlayMode->ShowWindow(SW_SHOW);
			//ex_pBtnActivatePlayMode->EnableWindow(TRUE);
			//ex_pBtnActivatePlayMode->UpdateWindow();
