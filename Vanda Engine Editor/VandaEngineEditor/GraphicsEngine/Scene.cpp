...
CInt CScene::WriteZipFile(CChar* zipFileName, CChar* fileInZipName, CChar* fileInZipPath, const CChar* password )
{
    ...
    if (err != ZIP_OK)
	{
		...
		zipClose(zipOpen, "Vanda Engine 2.9.0");
		free(buf);
		return -1;
	}
   // else
   ...
   //     {
			...
			//PrintInfo( temp, COLOR_RED );
			//zipCloseFileInZip(zf);
			//zipClose(zf, "Vanda Engine 2.9.0");
			//free(buf);
			//return -1;
   //     }
   // }
		...
		for( CUInt i = 0; i < m_instanceGeometries.size(); i++ )
		{
			...
			if( tempScene )
			{
				for( CUInt j = 0; j < tempScene->getNbActors(); j++ )
				{
					...
					{
						for(CInt nItem =0 ; nItem <  ex_pVandaEngineDlg->m_listBoxPhysXElements.GetItemCount(); nItem++)
						{
							CString strText = ex_pVandaEngineDlg->m_listBoxPhysXElements.GetItemText(nItem, 0);
