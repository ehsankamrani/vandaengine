...
void CEditProjectProperties::OnOK()
{
	...
	else
	{
		...
		if( m_edit )
		{
			...
			CChar temp[256];
			sprintf(temp, "%s%s%s%s%s", "Vanda Engine 2.1.0 (", newProjectName, " - ", m_currentVSceneNameWithoutDot, ")");
		...
		CChar temp[256];
		sprintf(temp, "%s%s%s%s%s", "Vanda Engine 2.1.0 (", newProject->m_name, " - ", m_currentVSceneNameWithoutDot, ")");
		ex_pVandaEngineDlg->SetWindowTextA(temp);
