...
BOOL CScriptUtilityGameObjects::OnInitDialog()
{
	...
	m_engineObjectListImage.Add(&cBmp, cBmpMask);
	cBmp.DeleteObject();
	cBmp.LoadBitmap(IDB_BITMAP_SCRIPT_UTILITY_VIDEO);
	m_engineObjectListImage.Add(&cBmp, cBmpMask);
	cBmp.DeleteObject();
		...
		if (Cmp(g_projects[pr]->m_name, m_projectName))
		{
			for (CUInt vs = 0; vs < g_projects[pr]->m_sceneNames.size(); vs++)
			{
				...
				if (Cmp(VSceneNameWithoutDot, m_VSceneName))
				{
					...
					for (CUInt j = 0; j < g_projects[pr]->m_vsceneObjectNames[vs].m_ambientSoundsNames.size(); j++)
					{
						gameObjectIndex++;
						int index = gameObjectIndex;
						LVITEM lvItem;
						...
						lvItem.iItem = index;
						lvItem.iSubItem = 0;
						lvItem.iImage = 1; //ambient sound
						lvItem.pszText = (CChar*)g_projects[pr]->m_vsceneObjectNames[vs].m_ambientSoundsNames[j].c_str();
						m_listGameObjectNames.InsertItem(&lvItem);
						m_listGameObjectNames.SetExtendedStyle(LVS_EX_INFOTIP | LVS_EX_ONECLICKACTIVATE | LVS_EX_LABELTIP);
					...
					for (CUInt j = 0; j < g_projects[pr]->m_vsceneObjectNames[vs].m_engineVideoNames.size(); j++)
					{
						gameObjectIndex++;
						int index = gameObjectIndex;
						LVITEM lvItem;
						...
						lvItem.iItem = index;
						lvItem.iSubItem = 0;
						lvItem.iImage = 6; //Video
						lvItem.pszText = (CChar*)g_projects[pr]->m_vsceneObjectNames[vs].m_engineVideoNames[j].c_str();
						m_listGameObjectNames.InsertItem(&lvItem);
						m_listGameObjectNames.SetExtendedStyle(LVS_EX_INFOTIP | LVS_EX_ONECLICKACTIVATE | LVS_EX_LABELTIP);
