...
void CEditPhysX::OnBnClickedBtnPhysx()
{
	...
	if( index != -1 )
	{
		...
		if( m_instanceGeometry &&  (m_fDensity > 0 || m_instanceGeometry->m_abstractGeometry->m_hasAnimation))
			ex_pVandaEngineDlg->InsertItemToPhysXList( m_instanceGeometry->m_physXName, ePHYSXELEMENTLIST_DYNAMIC_RIGIDBODY );
		else if( m_instanceGeometry && m_bIsTrigger )
			ex_pVandaEngineDlg->InsertItemToPhysXList( m_instanceGeometry->m_physXName, ePHYSXELEMENTLIST_TRIGGER );
		else if( m_instanceGeometry )
			ex_pVandaEngineDlg->InsertItemToPhysXList( m_instanceGeometry->m_physXName, ePHYSXELEMENTLIST_STATIC_RIGIDBODY );
...
CVoid CEditPhysX::RemovePhysXMesh()
{
	...
	for( CUInt i = 0; i < g_scene.size(); i++ )
	{
		for (int nItem = ex_pVandaEngineDlg->m_listBoxPhysXElements.GetItemCount()-1; nItem >= 0 ;nItem-- )
		{
			TCHAR szBuffer[1024];
			DWORD cchBuf(1024);
			LVITEM lvi;
			...
			ex_pVandaEngineDlg->m_listBoxPhysXElements.GetItem(&lvi);
			if( m_instanceGeometry && Cmp( szBuffer, m_instanceGeometry->m_physXName ) )
			{
				ex_pVandaEngineDlg->m_listBoxPhysXElements.DeleteItem(nItem);
				ex_pVandaEngineDlg->SortPhysXList();
				break;
			}
		}
...
void CEditPhysX::OnBnClickedCancel()
{
	...
	if (!Cmp((LPCTSTR)init_strPhysX, "\n"))
	{
		...
		if (m_instanceGeometry && (init_density > 0 || m_instanceGeometry->m_abstractGeometry->m_hasAnimation))
			ex_pVandaEngineDlg->InsertItemToPhysXList((CChar*)init_strPhysX.GetBuffer(init_strPhysX.GetLength()), ePHYSXELEMENTLIST_DYNAMIC_RIGIDBODY);
		else if (m_instanceGeometry && init_trigger)
			ex_pVandaEngineDlg->InsertItemToPhysXList((CChar*)init_strPhysX.GetBuffer(init_strPhysX.GetLength()), ePHYSXELEMENTLIST_TRIGGER);
		else if (m_instanceGeometry)
			ex_pVandaEngineDlg->InsertItemToPhysXList((CChar*)init_strPhysX.GetBuffer(init_strPhysX.GetLength()), ePHYSXELEMENTLIST_STATIC_RIGIDBODY);
	}
	g_updateOctree = CTrue;
	g_multipleView->m_nx->gControllers->reportSceneChanged();
	gPhysXscene->simulate(1.0f / 60.0f/*elapsedTime*/);
