...
CVoid CScene::Render(CBool sceneManager, CChar* parentTreeNameOfGeometries, CBool checkVisibility, CBool drawGeometry)
{
	if ( m_sceneRoot )
	{
		m_sceneRoot->Render(sceneManager, parentTreeNameOfGeometries, CFalse, checkVisibility, drawGeometry);
	}
}
...
CInt CScene::GeneratePhysX(CPhysXAlgorithm algorithm, CFloat density, CInt percentage, CBool isTrigger, CBool isInvisible, CPhysXMaterial physicsMaterial, CInstanceGeometry* m_instanceGeo, CBool loadFromFile, CInstancePrefab* prefab)
{
	...
	{
		if (!g_nx->CreateTriangleMesh((CInt)(vertices_temp.size() / 3), (CInt)(triangles_temp.size() / 3), vertices, triangles, isTrigger, m_instanceGeo->m_physXName, physicsMaterial))
			failed = CTrue;
		else
	...
	{
		if (!g_nx->CreateConvexMesh((CInt)(vertices_temp.size() / 3), vertices, NxVec3(convexPosition.x, convexPosition.y, convexPosition.z), NxConvexMat33Rotation, density, m_instanceGeo->m_physXName, isTrigger, m_instanceGeo->m_abstractGeometry->m_hasAnimation, physicsMaterial))
			failed = CTrue;
		else
			sprintf(m_instanceGeo->m_physXName, "%s%s%d", g_currentInstancePrefab->GetName(), "_PhysX_mesh_", m_instanceGeo->m_nameIndex);
	}
	...
	else if (algorithm == eLOD_BOX || algorithm == eLOD_TRIGGER)
	{
		...
		if (algorithm == eLOD_BOX)
			g_nx->CreateBox(NxVec3(PosX, PosY, PosZ), NxVec3(DimX, DimY, DimZ), density, NxConvexMat33Rotation, m_instanceGeo->m_physXName, isTrigger, m_instanceGeo->m_abstractGeometry->m_hasAnimation, physicsMaterial);
		else
			g_nx->CreateTriggerBox(NxVec3(PosX, PosY, PosZ), NxVec3(DimX, DimY, DimZ), NxConvexMat33Rotation, m_instanceGeo->m_physXName, m_instanceGeo->m_abstractGeometry->m_hasAnimation, physicsMaterial);
		sprintf(m_instanceGeo->m_physXName, "%s%s%d", g_currentInstancePrefab->GetName(), "_PhysX_mesh_", m_instanceGeo->m_nameIndex);
		m_instanceGeo->m_lodAlgorithm = algorithm;
		m_instanceGeo->m_hasPhysX = CTrue;
		m_instanceGeo->m_physXDensity = density;
	...
	else if (algorithm == eLOD_SPHERE)
	{
		...
		g_nx->CreateSphere(NxVec3(PosX, PosY, PosZ), radius, density, m_instanceGeo->m_physXName, isTrigger, m_instanceGeo->m_abstractGeometry->m_hasAnimation, physicsMaterial);
		sprintf(m_instanceGeo->m_physXName, "%s%s%d", g_currentInstancePrefab->GetName(), "_PhysX_mesh_", m_instanceGeo->m_nameIndex);
		m_instanceGeo->m_lodAlgorithm = algorithm;
		m_instanceGeo->m_hasPhysX = CTrue;
		m_instanceGeo->m_physXDensity = density;
	...
	else if (algorithm == eLOD_CAPSULE_METHOD1 || algorithm == eLOD_CAPSULE_METHOD2 || algorithm == eLOD_CAPSULE_METHOD3)
	{
		...
		g_nx->CreateCapsule(NxVec3(PosX, PosY, PosZ), height, radius, density, NxCapsuleMat33Rotation, m_instanceGeo->m_physXName, isTrigger, m_instanceGeo->m_abstractGeometry->m_hasAnimation, physicsMaterial);
		sprintf(m_instanceGeo->m_physXName, "%s%s%d", g_currentInstancePrefab->GetName(), "_PhysX_mesh_", m_instanceGeo->m_nameIndex);
		m_instanceGeo->m_lodAlgorithm = algorithm;
		m_instanceGeo->m_hasPhysX = CTrue;
		m_instanceGeo->m_physXDensity = density;
